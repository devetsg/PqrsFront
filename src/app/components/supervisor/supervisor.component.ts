...
export class SupervisorComponent {
  ...
  showSend(){
    this.dataSource.data = this.allPqrs.filter((x:any)=> x.status == "SEND")
  }

  showPending(){
    this.dataSource.data = this.allPqrs.filter((x:any)=> x.status == "PENDING")
  }

  showReview(){
    this.dataSource.data = this.allPqrs.filter((x:any)=> x.status == "COORD")
  }
      ...
      next: (data: any) => {
        ...
        if (data.message == "FALSE") {
          Swal.fire({
            ...
            text: 'Su usuario no cuenta con una firma asignada, por favor subir su firma digital',
            showConfirmButton: true,
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
              this._redirect.navigate(['/signatures'])
            }
          })
        }
      }
